	...
	return function (target, formOps, opts) {
		...
		form.data = function (keyOrObj, value) {
			if ( typeof keyOrObj === 'string' && typeof value !== 'undefined' ) {
				fn.path.set(keyOrObj, value, form.datas);
				return form;
			} else if ( typeof keyOrObj === 'string' && typeof value === 'undefined' ) {
				return fn.path.get(keyOrObj, form.datas);
			}
			fn.mergeObject(form.datas, keyOrObj);
			return form;
		};
		...
		form.begin = function (pTarget, pOpts) {
			...
			return fn.form.begin(pTarget || form.settings.action, pOpts || form.formSettings );
		};
